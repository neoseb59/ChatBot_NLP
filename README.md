...
> **Please note:** The current implementation of the question generator script produces results of questionable quality. If you check the line with a `HACK:` tag, you will see why: most context prompts/paragraphs (which are just responses from the source website, as discussed) are *too long* for the transformer model which tokenizes them and generates questions from them. To fix the issue and allow the code to run, we have decided that all paragraphs shall be split down into individual **sentences** (around the `. ` substring), hence the *hack*. This cannot be fixed using the current pre-trained transformers. Using custom trained ones (or just different transformers altogether) might fix the issue, but until then, prompts are very short and the questions are thus very **numerous** and rather **bad**. This issue has not been addressed.

---

Speaking of which, to train and run the LLM itself, you need access to a repo in a [*Hugging Face*](https://huggingface.co/) account of your own. You can decide to attempt **fine-tuning** it [*locally*](#fine-tuning-the-model-locally) if you have enough RAM and a decent GPU, or you can try and see for [*online*](#fine-tuning-the-model-online) options (which may sometimes require payment); we provide a link to a Google Colab for basic training using a Jupyter Notebook.
